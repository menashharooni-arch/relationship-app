...
function Row({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="text-[0.8125rem] text-slate-600 truncate">{children}</span>
    </div>
  );
}
    ...
    >
      ...
      <div className="relative h-[92px]" style={{ background: `linear-gradient(120deg, ${accent}, ${accent2})` }}>
        ...
        <div className="absolute top-3.5 left-4 right-4 flex items-center justify-between">
          <span className="text-white/90 text-[0.625rem] font-bold tracking-[0.18em] uppercase">SwiftCard</span>
          <span className="text-white/80 text-[0.625rem] font-semibold">{company}</span>
        </div>
      </div>
      ...
      <div className="px-5 pb-5">
        ...
        <div className="-mt-10 mb-3 flex items-end justify-between">
          <div className="w-[72px] h-[72px] rounded-2xl overflow-hidden border-[3px] border-white shadow-md bg-slate-100 flex items-center justify-center">
            {photoUrl ? (
              ...
              <span className="text-[1.375rem] font-black" style={{ color: accent }}>{initials}</span>
            )}
          </div>
        ...
        <p className="text-[1.1875rem] font-extrabold text-slate-900 leading-tight tracking-tight">{name}</p>
        <p className="text-[0.8125rem] font-medium" style={{ color: accent }}>{title}</p>
        ...
        {/* Save contact */}
        <div className="mt-5 flex items-center justify-center gap-2 rounded-full py-2.5 text-white text-[0.8125rem] font-bold" style={{ background: accent, boxShadow: `0 8px 20px -8px ${accent}` }}>

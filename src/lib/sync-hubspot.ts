import { getCrmConnection, setSyncError, connectionErrorMessage, resolveCrmOwnerId, describeCapture, shouldAttachNote, type CrmLead, type CrmSyncOptions } from "./crm-connection";
...
export async function syncLeadToHubSpot(lead: CrmLead, capturedBy: string, opts?: CrmSyncOptions): Promise<void> {
  ...
  const conn = await getCrmConnection("hubspot", LABEL, userId, lead.capturedByCardId, capturedBy, {
  ...
  if (res.ok) {
    ...
    try {
      ...
      if (shouldAttachNote(opts)) await attachContextNote(conn.token, created?.id, lead);
  ...
  if (res.status === 409 && lead.email) {
    ...
    try {
      ...
      if (shouldAttachNote(opts)) await attachContextNote(conn.token, updated?.id, lead);
    } catch { /* note is a bonus, never a failure */ }
    if (conn.syncError) await setSyncError("hubspot", userId, null);
    return;
  }

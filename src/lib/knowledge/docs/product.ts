...
export const productDocs = defineDocs([
  ...
  },
  {
    ...
    detail:
      "You set the bio, socials and extra buttons on the Socials step of the card builder (step 3), or the \"Socials\" tab when editing a card. How the page LOOKS is the separate \"Social design\" step/tab: pick a named Look — one tap sets the whole scheme. Every page defaults to the light \"Paper\" Look; Free can also choose the dark \"Onyx\", while the full Look library (Blush, Sand, Chrome, Forest, Midnight, Orchid) and the custom background/text/font pickers are Pro. Free allows {limit.links} additional link buttons, shown as slim rows; paid plans are unlimited and each link gets a tile-size control (Auto, Featured full-width, Grid half-width pairs, or Compact rows) next to it in the editor. Video previews, featured tiles and the grid are Pro.",
  },
  {
    id: "sharing-a-card",

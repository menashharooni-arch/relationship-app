...
export const accountDocs = defineDocs([
  ...
  },
  {
    ...
    answer:
      "Settings → Notifications and preferences. The push switch at the top turns on instant alerts; directly under it, \"What we notify you about\" lists exactly what SwiftCard is allowed to send you — new contacts, replies, the first view of a card, and billing problems — each with its own switch, plus a \"Quiet hours\" switch. Those save the moment you tap them. Below that are the email switches — \"Marketing emails\" for everyone, and \"Payment receipts\" for anyone who is actually billed; those two need \"Save preferences\" pressed.",
    detail:
      "What can reach your phone is a short, fixed list and nothing else: someone shares their details, a contact replies to one of your follow-ups, one of your cards is opened for the very first time, and a failed payment. No tips, no promotions, no weekly stats, no \"you hit 100 views\" \u2014 those still appear in the bell inside the app and in email. Every plan gets the same notifications, free accounts included. Repeat views of the same card don't notify; a first view is also limited to one an hour so a printed QR code at an event can't set off a stream of alerts. Away from leads and billing there is a limit of five a day. Quiet hours hold everything between 10pm and 8am your own time \u2014 a billing problem is the one exception, because it's time-critical and rare. The email switches are NOT auto-saved — a common complaint is that the toggle looked flipped but nothing changed, which means \"Save preferences\" was never pressed. Every marketing email also carries an unsubscribe link that lands on /unsubscribe and works without signing in. Payment receipts and other transactional mail keep sending regardless. On a team, only the person who pays sees the \"Payment receipts\" switch at all: an Office member's seat is paid by the office owner, so they are never billed and never receive a receipt — the switch is hidden for them rather than offering control over mail that cannot arrive. It still shows for the office owner, for a member given the billing-admin role, and for a member who kept their own personal subscription from before joining the team, because those three really are charged. In the iPhone app the push switch is also offered on the \"Your card is live!\" screen after a card is created, and as a \"Know the moment someone connects\" box at the top of the dashboard the first time someone opens the app signed in (it goes away once the switch is on, or after \"Not now\"). If the app says \"This version of SwiftCard can't receive notifications yet\", the fix is to update the app from the App Store — tapping the switch again won't help. If it says notifications are turned off for SwiftCard, iPhone Settings → SwiftCard → Notifications → Allow Notifications is the only way back; the app cannot ask a second time.",
  },
  {

...
export const cardDocs = defineDocs([
  ...
  },
  {
    ...
    answer:
      "Settings → Cards and sharing → Edit → the \"Card design\" tab. There are {product.templateCount} templates — Classic Pro, Modern Bold, Photo First, Local Business, Luxury Minimal and Logo First — and every one of them is available on every plan. Pick one, then restyle it in the panel below, which runs in three parts. \"Looks\" comes first — one tap sets the whole card. Then \"Colour\" (the background, the second panel where a template has one, and your name colour), then \"Style\" (Font, then Finish). The three least-used controls — the details colour, the accent colour, and the panel photo or video — sit under a \"More style options\" row you tap to open; nothing is removed, it is just folded away. Under Style there is a \"Finish\" — the material laid over your colour, in three groups: Plain (Flat, the standard look), Light (Sheen, a band of light; Halo, a glow behind your logo; Frosted, a milky sheet) and Material (Brushed metal, Carbon, Linen and Gilt edge, a foil rule). Flat, Sheen and Halo are on every plan; the other five come with Pro. A finish reads strongest on deeper colours, and Frosted lightens the panel, so the editor warns that a white name may need a darker colour. Inside \"More style options\", \"Photo or video\" (Pro) puts a photo or a short video behind that same surface, with a \"Darken\" slider so your name stays readable. A video plays on your live card page; everywhere a video cannot play — the card image you download or share, link previews, and your email signature — its first frame is shown instead, so the card never looks empty. Finishes work on all six templates and change nothing on a card that has not picked one.",
    detail:

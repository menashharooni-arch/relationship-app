...
export const cardDocs = defineDocs([
  ...
  },
  {
    ...
    answer:
      "Card editor → Social design → set \"Page header\" to \"Compact circle\" or \"No header\", then use \"Add\" under \"Page background\" to upload a photo or a short video. It fills the whole page behind your links. Photos up to 5 MB, videos up to 25 MB — the video plays muted and loops on its own.",
    detail:
      "The buttons only appear with the Compact circle or No header layouts. Cover photo and Short banner already lead with a big photo at the top, so a second full-page image would fight it — pick a background colour there instead. Two controls appear once a background is set. \"Darken\" dims the photo so your name and links stay readable; nudge it up if a bright photo is washing out the text. \"Blur the link buttons\" turns the plain rows into frosted glass that your background shows softly through — it is on by default when you add a background, and it makes a real difference over a busy photo. Both are Pro, along with the background itself. If you change the Page header afterwards the background is kept, not deleted: the editor says so, and switching back to Compact circle or No header brings it straight back. Over a background your name and link labels are shown in white automatically, unless you picked a text colour yourself. Portrait shots fit best — the picture is cropped to fill the page, so a wide landscape photo shows only its middle on a phone.",
  },
  {
    id: "card-url",

...
| Apple token revocation on account deletion | **Not working.** `identity_data` carries ID-token claims, not provider tokens; `provider_refresh_token` is returned on the session at sign-in and nothing persists it, so `revokeAppleTokensOnDelete` returns `no_token` and Apple's revoke endpoint is never called | — | Account deletion itself works fully (data purged). App Review 5.1.1(v) expects revocation for Sign-in-with-Apple accounts. Fix: persist `session.provider_refresh_token` in `/auth/callback`. No Apple accounts exist yet — provider went live 2026-08-07 |

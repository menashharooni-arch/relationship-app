...
| Dedicated watchOS app | Roadmap — does not exist | — | Wallet pass syncs to Watch Wallet (real since 2026-08-10 — see the Apple Wallet row); in-app clarifier on /products/watch says exactly this |
| Apple Wallet passes | **Live since 2026-08-10.** Pass Type ID `pass.me.swiftcard.card`; `/api/wallet/pass` serves a signed `.pkpass` (signature verified against the WWDR G4 chain) | — | Still gated by `hasWalletConfig()`, and the uptime health check probes the endpoint — if the cert or env ever breaks, an issue opens before a reviewer finds it |
| Apple token revocation on account deletion | **Fixed 2026-08-10.** The provider refresh token is captured at sign-in (`/auth/callback` server-side; `/api/auth/apple-token` for the native shell, where the PKCE exchange must run in the webview) into `apple_refresh_tokens` (service-role-only RLS, ON DELETE CASCADE), and `revokeAppleTokensOnDelete` reads it from there | — | 5.1.1(v) satisfied for every Apple account created from now on. Table + posture recorded in `supabase/apple-refresh-tokens.sql` |

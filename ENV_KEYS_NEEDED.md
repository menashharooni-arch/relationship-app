...
| `MARKETING_FROM_EMAIL` | **Deliverability.** Sender for campaigns only (`promoEmail`, `marketingEmail`), e.g. `SwiftCard <news@news.swiftcard.me>`. Verify a second subdomain in Resend and publish its DNS first. Reputation is scored per subdomain, so this keeps a campaign that draws complaints from also sending receipts and payment-failure warnings to spam. Unset = falls back to the transactional sender (no isolation). **Set it before the first campaign.** |
